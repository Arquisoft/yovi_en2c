...
performance.
* **For bot developers**: A public interoperability API (botapi) that allows automated bots to interact with the game engine, request moves, and
play matches.
...
From a technical perspective, the YOVI system is implemented as multiple independent services communicating via HTTP/REST. Public access is handled through an **Nginx reverse proxy**, which routes external requests to the appropriate services (webapp, gateway, botapi). This section describes the technical channels and
...
node "YOVI Deployment" {
  frame "DMZ / Gateway" {
    ...
    component "API Gateway (Web Backend)" as Gateway
    component "Bot API Service" as BotAPI
  }
...
Bot --> BotClient
BotClient --> Nginx : HTTPS (port 443)

Nginx --> Webapp : HTTP (internal)
Nginx --> Gateway : HTTP /api/*
Nginx --> BotAPI : HTTP /interop/*
...
| **Game Play UI** | Browser ↔ Nginx ↔ Webapp | HTTPS | HTML/CSS/JS + JSON
...
| **Bot API - Move Request** | Bot ↔ Nginx ↔ BotAPI ↔ Gamey | HTTPS → HTTP → JSON | JSON (YEN)
| **Bot API - Game State** | Bot ↔ Nginx ↔ BotAPI ↔ Gamey | HTTPS → HTTP → JSON | JSON (YEN)
...
| **Reverse Proxy** | Nginx | HTTPS termination and request routing
| **API Gateway** | Node.js + Express | Backend entry point for webapp
| **Bot API Service** | Node.js + Express | Public interoperability API for bots
...
* Public interoperability API (botapi) for bots with YEN notation
