...
With the addition of a **reverse proxy** (e.g., Nginx) in front of the API Gateway:

|=== 
| Layer | Measure | Purpose
| **Transport** | HTTPS terminated at Nginx Reverse Proxy | Encrypt all external traffic
| **Internal Traffic** | HTTP from Proxy → API Gateway → Services | Simplified internal routing; services never exposed externally
| **Network isolation** | Docker networks (frontend/backend) | Prevent direct backend access
| **Firewall** | Only ports 22 (SSH) and 443 (HTTPS) exposed externally | Minimize attack surface
| **API Gateway** | Single internal entry point behind proxy | Route requests internally, hide service topology, log/monitor requests
|===
