...
....
flowchart TD
    User["Human User"] -->|Uses Web Interface| WebApp
    Bot["External Bot"] -->|Calls Public Bot API| WebApp

    WebApp["yovi_web_app<br/>TypeScript/React"] -->|Game Management API<br/><i>YEN/JSON over HTTP</i>| Engine
    Engine["yovi_game_engine<br/>Rust Service"]

    classDef frontend fill:#dae8fc,stroke:#6c8ebf
    classDef backend fill:#d5e8d4,stroke:#82b366
    classDef actor fill:#f5f5f5,stroke:#666

    class WebApp frontend
    class Engine backend
    class User,Bot actor
....
...
*   **Open Issues/Problems/Risks**: Performance of complex AI strategies on large boards. Learning curve for Rust (See ADR 3 and Section 11).
...
....
flowchart TD
    subgraph WebApp[yovi_web_app]
        UI[UI Layer<br/><i>React Components</i>]
        API[API Client & State Manager]
        Router[Public API Router]
    ...
    UI -->|Renders game board<br/>Implements i18n & responsive| UserInterface[Web Browser]
    Router -->|Exposes /api/v1/*<br/>Handles bot auth| ExternalBot[External Bot]
    API -->|Calls| GameEngine[yovi_game_engine]

    classDef component fill:#f5f5f5,stroke:#666,color:#333
    classDef external fill:#e1d5e7,stroke:#9673a6

    class UI,API,Router,Service component
    class UserInterface,ExternalBot,GameEngine external

    style WebApp fill:#fff,stroke:#6c8ebf,stroke-width:2px
....
...
....
flowchart LR
    subgraph Engine[yovi_game_engine]
        Parser[YEN Parser & Validator]
        Rules[Game Rules Core]
        AI[AI Strategy Module]

        Parser -->|Validated game state| Rules
        Rules -->|Requests move| AI
        AI -->|Returns move| Rules
    end

    WebApp[yovi_web_app] -->|Sends YEN JSON| Parser
    Rules -->|Returns YEN response| WebApp

    classDef component fill:#f5f5f5,stroke:#666,color:#333
    classDef external fill:#dae8fc,stroke:#6c8ebf

    class Parser,Rules,AI component
    class WebApp external

    style Engine fill:#fff,stroke:#82b366,stroke-width:2px
....
