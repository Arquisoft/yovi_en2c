...
describe("Gateway Service", () => {
  ...
  describe("Auth endpoints", () => {
    it("POST /register forwards request correctly", async () => {
      axios.post.mockResolvedValueOnce({
        status: 201,
        data: {
          success: true,
    ...
    it("POST /register propagates auth-service error response", async () => {
      axios.post.mockRejectedValueOnce({
        response: {
          status: 400,
          data: { error: "Password and repeat password must be equal" }
        }
      });
    ...
    it("POST /register returns 502 if auth-service fails", async () => {
    ...
    it("POST /login propagates auth-service error response", async () => {
      axios.post.mockRejectedValueOnce({
        response: {
          status: 401,
    ...
    it("GET /verify forwards Authorization header correctly", async () => {
      axios.get.mockResolvedValueOnce({
        status: 200,
        data: {
          success: true,
    ...
    it("GET /verify propagates auth-service error response", async () => {
      axios.get.mockRejectedValueOnce({
        response: {
          status: 401,
    ...
    it("GET /verify returns 502 if auth-service fails", async () => {
