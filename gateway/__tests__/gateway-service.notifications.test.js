    ...
    it("returns 200 with notifications and unreadCount", async () => {
        axios.get.mockResolvedValueOnce({
            ...
            data: {
                success: true,
                notifications: [
                    { id: "abc", type: "welcome", from: null, read: false, createdAt: "2026-04-01" },
                ],
                unreadCount: 1,
            },
        });
...
describe("Gateway — PATCH /notifications/:id/read", () => {
    ...
    it("returns 200 and forwards the request to users service", async () => {
        axios.patch.mockResolvedValueOnce({
            ...
            data: {
                success: true,
                notification: {
                    id: validId, type: "welcome", from: null,
                    read: true, createdAt: "2026-04-01",
                },
            },
        });
    ...
    it("forwards sanitized Authorization header to users service", async () => {
        ...
        expect(axios.patch).toHaveBeenCalledWith(
            ...
            {},
            expect.objectContaining({
                headers: expect.objectContaining({ Authorization: "Bearer my_read_token" }),
            })
        );
    });
