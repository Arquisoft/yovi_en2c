import { InteropStatus } from "../models/active-game.model";
import { CoordinatesDto, YenDto } from "../dtos/yen.dto";

interface PvbResultLike {
  yen: YenDto;
  finished: boolean;
  winner: string | null;
}
...
export function assertValidYen(yen: YenDto): void {
  if (!yen) {
  ...
  for (let row = 0; row < rows.length; row += 1) {
    ...
    if (rows[row].length !== expectedLength) {
      throw new Error(
        `YEN row ${row} must have length ${expectedLength}, got ${rows[row].length}`
      );
    }
  }
}

export function detectSingleAddedMove(previous: YenDto, proposed: YenDto): DiffMove {
  ...
  let detected: DiffMove | null = null;
  ...
  for (let row = 0; row < oldRows.length; row += 1) {
    for (let col = 0; col < oldRows[row].length; col += 1) {
      ...
      if (newCell !== currentPlayerToken) {
        throw new Error(
          `proposed position must add exactly one '${currentPlayerToken}' token`
        );
      }
...
export function applyMoveToYen(yen: YenDto, coords: CoordinatesDto): YenDto {
  assertValidYen(yen);
...
export function computeStatusFromPvbResponse(result: PvbResultLike): InteropStatus {
  if (!result.finished) {
    return "ONGOING";
  }
...
export function normalizeStatus(status: InteropStatus): InteropStatus {
  return status;
}

function splitRows(layout: string): string[][] {
