...
function renderGame(username = "Pablo", bot = "minimax_bot", boardSize = 7) {
  localStorage.clear();
  ...
  return render(
      <I18nProvider>
        <MemoryRouter initialEntries={[{
          pathname: "/game",
          // timerSeconds: 0 → timer disabled, keeps existing tests unchanged
...
describe("Game — saveGameResult and boardSize", () => {
  ...
  test("sends boardSize from state to POST /game/new", async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(
          ...
          expect.objectContaining({ body: JSON.stringify({ size: 9 }) })
      );
    });
  });
  ...
  test("sends default boardSize 7 when not in state", async () => {
    ...
    render(
        <I18nProvider>
          <MemoryRouter initialEntries={[{
            pathname: "/game",
            state: { username: "Pablo", bot: "minimax_bot", timerSeconds: 0 },
          } as any]}>
            <Game />
          </MemoryRouter>
        </I18nProvider>
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(
          expect.stringMatching(/\/game\/new$/),
          expect.objectContaining({ body: JSON.stringify({ size: 7 }) })
      );
    });
  });
  ...
  test("calls POST /gameresult with correct fields when player wins", async () => {
    ...
    expect(body).toMatchObject({
      username: "Pablo", opponent: "minimax_bot", result: "win", boardSize: 7, gameMode: "pvb",
    });
    expect(body.score).toBeGreaterThanOrEqual(1);
    // Normal win should NOT have endReason
    expect(body.endReason).toBeUndefined();
  });
  ...
  test("does NOT call POST /gameresult when game ends in draw", async () => {
    ...
    await waitFor(() => { expect(document.querySelectorAll("polygon").length).toBeGreaterThan(0); });
    await user.click(document.querySelectorAll("polygon")[0]);
    await waitFor(() => { expect(screen.getByText(/Empate|Draw/i)).toBeInTheDocument(); });
  ...
  test("resets moveCount to 0 when new game is started", async () => {
    ...
    const body = JSON.parse((global.fetch as any).mock.calls[4][1].body);
